...
BoardArray2D::BoardArray2D()
{
    ...
    arr = new int *[m_rowsize];
    for (unsigned i = 0; i < m_rowsize; ++i)
        arr[i] = new int[m_colsize];
...
*/
bool BoardArray2D::readFromFile(std::ifstream &file)
{
    const int EMPTY = -1;
    const int BLOCK = -2;
    bool success = true;
...
*/
bool BoardArray2D::writeToFile(std::ofstream &file) const
{
    const int EMPTY = -1;
    const int BLOCK = -2;
    bool success = true;
 ...
 * Call reset()
*/
void BoardArray2D::setSize(unsigned int row, unsigned int col)
{
    // Delete old array
    for (unsigned i = 0; i < m_rowsize; ++i)
        delete[] arr[i];
...
*/
bool BoardArray2D::move(char move)
{
    const int EMPTY = -1;
    const int BLOCK = -2;
    bool isMoved = false;
...
*/
int BoardArray2D::operator()(unsigned int row, unsigned int col) const
{
    const int BLOCK = -2;
