...
char getInput() {
	...
	while(input != 'I' && input != 'N' && input != 'S' && input != 'M') {

		printf("Enter sorting metric(I=id N=name S=surname M=mail): ");
		scanf("%c", &input);
...
int main() {
	...
	/* Flag for bubble sort algorithm */
	int swapped;

	/* Do bubble sort algorithms depending on input */
	switch(getInput()) {
		case 'I':
			do {
				Person tmp;
		...
		case 'S':
			do {
				...
				swapped = 0;
				for(int i = 0; i < recordSize - 1; i++) {
					if(strcmp(records[i].surname,records[i+1].surname) > 0) {
						tmp = records[i];
						records[i] = records[i+1];
						records[i+1] = tmp; 
						swapped = 1;
		...
		case 'M':
			do {
				...
				swapped = 0;
				for(int i = 0; i < recordSize - 1; i++) {
					if(strcmp(records[i].email,records[i+1].email) > 0) {
						tmp = records[i];
						records[i] = records[i+1];
						records[i+1] = tmp; 
						swapped = 1;
